      ...
      {employee && (
        <>
            ...
            <div className="flex items-start gap-5">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider text-teal-700 bg-teal-50 border border-teal-200">
                    Active
                  </span>
                </div>
              ...
              <Link to="/employees" className="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors shrink-0">
                ← Back
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              <h2 className="text-sm font-bold text-slate-900 mb-3">Mark Attendance</h2>
              <form onSubmit={handleMarkAttendance} className="flex flex-wrap gap-3 items-end">
                <div>
                  ...
                  <input type="date" value={date} onChange={e => setDate(e.target.value)} required
                    className="border border-slate-200 rounded-lg px-3 py-2 text-sm bg-slate-50 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500" />
                </div>
                <div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              <h2 className="text-sm font-bold text-slate-900 mb-3">Filter Records</h2>
              <form onSubmit={handleFilter} className="flex flex-wrap gap-3 items-end">
                ...
                <button type="submit" className="px-4 py-2 rounded-lg text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">
                  Apply
                </button>
                {(startDate || endDate) && (
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              ...
              <span className="text-[11px] text-slate-400">{records.length} record{records.length !== 1 ? 's' : ''}</span>
            </div>
            {loading ? (
              ...
              <div className="py-16 text-center">
                ...
                <p className="text-xs text-slate-300 mt-1">Mark attendance above to get started.</p>
              </div>
            ) : (
                ...
                <table className="min-w-full">
                  <thead>
                    <tr className="border-b border-slate-100">
                      ...
                      <th className="px-5 py-3 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Day</th>
                        ...
                        <tr key={record.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-5 py-3">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-medium ${
                              record.status === 'Present' ? 'bg-teal-50 text-teal-700' : 'bg-rose-50 text-rose-700'
                            }`}>
